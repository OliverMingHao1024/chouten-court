...
> ⚠️ **已停用,不代表現況(僅供歷史存查)**:本文件對照的是補完 7 個落差項目當時的 `docs/spec.md` 與程式碼狀態,之後訓練系統、球員個性效果等均已多次變動(例如訓練已從三檔風險機率→骰子制→訓練卡池制;隊長型/抗壓型效果已於後續階段補上),本文件**未同步更新**,內容已與現況不符。
>
> 目前進度請改看 `docs/baseball-modes-benchmark-and-optimization-plan.md` 第 12 節(V1~V4 checklist,持續維護中)。**請勿依本文件內容做判斷或實作決策。**
